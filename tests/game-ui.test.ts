...
async function setup(t: any, options: { pending?: Operation; fail?: boolean; schema?: number; storageFails?: boolean } = {}) {
  ...
  const source = ['src/game/rules.ts', 'src/game/api.ts', 'src/game/index.ts'].map(path => stripTypeScriptTypes(readFileSync(path, 'utf8').replace(/^import .*;\n/gm, '').replace(/^export /gm, ''))).join('\n');
  ...
  await tick();
  return { window, find, controller: (window as any).controller, sent, setFail: (v: boolean) => { fail = v; }, setState: (s: Snapshot) => { state = s; } };
}
async function quietRound(ui: Awaited<ReturnType<typeof setup>>) {
  ...
  ui.find('[data-action="quiet"]').click();
...
test('Pause und Verlassen erhalten erledigte Aktionen; Fortsetzen ist ausdrücklich nötig', async t => {
  ...
  ui.find('[data-game="pacifier"]').click(); ui.find('[data-action="begin"]').click(); await tick();
  ui.find('[data-action="quiet"]').click(); ui.find('[data-answer]').click(); ui.find('[data-action="next"]').click();
