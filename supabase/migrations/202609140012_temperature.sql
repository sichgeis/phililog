alter table public.feedings add column temperature_c numeric;
alter table public.feedings drop constraint feedings_kind_check;
alter table public.feedings add constraint feedings_kind_check check (kind in ('bottle', 'breast', 'diaper', 'weight', 'temperature'));
...
grant insert (temperature_c), update (temperature_c) on public.feedings to authenticated;
