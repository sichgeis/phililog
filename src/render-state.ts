...
export function preserveFormState(root: HTMLElement): () => void {
  ...
  const details = [...root.querySelectorAll<HTMLDetailsElement>('form details')].map(el => el.open);
  return () => {
    root.querySelectorAll<HTMLDetailsElement>('form details').forEach((el, i) => { if (details[i] !== undefined) el.open = details[i]; });
