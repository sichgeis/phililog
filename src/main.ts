...
function restore() {
  try {
    ...
    if (saved && ['bottle', 'breast', 'diaper', 'weight', 'temperature'].includes(saved.draft?.kind)) {
...
function bindForm() {
  ...
  document.querySelectorAll<HTMLButtonElement>('[data-kind]').forEach(b => b.addEventListener('click', () => {
    ...
    if (draft.kind === 'weight' || draft.kind === 'temperature') document.querySelector<HTMLElement>('#extra-events summary')?.focus({ preventScroll: true });
  }));
