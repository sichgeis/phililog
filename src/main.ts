...
function header() {
  return `<header class="topbar"><a class="brand" href="./" aria-label="Phililog Startseite"><span class="brand-mark">p<span></span></span>phililog<span class="brand-dot">.</span></a><span class="private-label"><span class="status-dot"></span>Euer kleines Logbuch</span>${userId ? '<button class="text-button logout" id="logout">Abmelden</button>' : ''}</header>`;
}
...
function formView(): string {
  ...
  return `<div class="page-heading"><div><span class="eyebrow">${edit ? 'EINTRAG KORRIGIEREN' : 'EIN MOMENT FÜR EUER LOGBUCH'}</span><h1>${edit ? 'Eintrag bearbeiten' : 'Eine kleine Mahlzeit.'}</h1><p>${edit ? 'Passt die Angaben an. Der ursprüngliche Eintrag wird aktualisiert.' : 'Kurz festhalten. Dann wieder ganz füreinander da sein.'}</p></div><span class="heading-flower" aria-hidden="true">✳</span></div><div class="form-layout"><section class="card feeding-card"><form id="feeding-form" novalidate><fieldset ${busy || pending ? 'disabled' : ''}><legend class="sr-only">Fütterung erfassen</legend><div class="field-header"><label>Wie wurde gefüttert?</label><span class="field-number">01</span></div><div class="kind-picker" role="group" aria-label="Fütterungsart"><button type="button" data-kind="bottle" ${draft.breastStart && !draft.breastEnd ? 'disabled' : ''} aria-pressed="${bottle}" class="kind-button ${bottle ? 'selected' : ''}">${icon('bottle')}<span>Flasche</span>${bottle ? icon('check') : ''}</button><button type="button" data-kind="breast" aria-pressed="${!bottle}" class="kind-button ${!bottle ? 'selected' : ''}">${icon('heart')}<span>Stillen</span>${!bottle ? icon('check') : ''}</button></div>
      ...
      <div class="field-block time-block" ${timed ? 'hidden' : ''}><div class="field-header"><label>Wann war die Mahlzeit zu Ende?</label><span class="field-number">02</span></div><div class="segmented" role="group" aria-label="Fütterungszeitpunkt"><button type="button" data-time="now" aria-pressed="${draft.timeMode === 'now'}" class="${draft.timeMode === 'now' ? 'active' : ''}" ${timed ? 'disabled' : ''}>Gerade eben</button><button type="button" data-time="custom" aria-pressed="${draft.timeMode === 'custom'}" class="${draft.timeMode === 'custom' ? 'active' : ''}" ${timed ? 'disabled' : ''}>Anderer Zeitpunkt</button></div>${draft.timeMode === 'custom' ? `<label class="sr-only" for="local-time">Datum und Uhrzeit</label><input id="local-time" ${timed ? 'disabled' : ''} type="datetime-local" step="60" value="${escape(draft.localTime)}" required>` : '<p class="field-note">Die aktuelle Uhrzeit wird beim Speichern eingetragen.</p>'}</div></fieldset>
      ...
      </form></section><aside class="aside"><section class="last-entry"><div class="section-title"><h2>Zuletzt gefüttert</h2><span class="small-dot"></span></div>${latest ? entryCard(latest, true) : `<div class="empty-mini">${icon('book')}<p>${loading ? 'Logbuch wird geladen …' : 'Hier erscheint eure letzte Mahlzeit.'}</p></div>`}<button id="see-history" class="history-link">Zum Logbuch ${icon('arrow')}</button></section><div class="gentle-note"><span aria-hidden="true">✦</span><p>Jeder kleine Eintrag<br>hilft euch, den Überblick zu behalten.</p></div></aside></div>`;
}
...
function switchView(next: typeof view) {
  if (busy) return;
  view = next; notice = ''; error = ''; render(); void refresh();
}
function bindLogout() {
