...
import { hasDraftChanges, sameInput, moods, type Mood, type Person, estimatedMilk, newDraft, draftFromFeeding, feedingInput, localDateTime, dayKey, elapsedLabel, sideLabel, milkLabel, kindLabel, toCsv, PAGE_SIZE, type Draft, type Feeding, type PendingCreate } from './domain.ts';
...
function restore() {
  try {
    ...
    if (saved && ['bottle', 'breast', 'diaper', 'weight', 'temperature', 'sunbath'].includes(saved.draft?.kind)) {
...
function bindForm() {
  ...
  document.querySelectorAll<HTMLButtonElement>('[data-kind]').forEach(b => b.addEventListener('click', () => {
    captureForm(); draft.kind = b.dataset.kind as Draft['kind']; if (draft.kind === 'weight' || draft.kind === 'temperature' || draft.kind === 'sunbath') draft.mood = null; remember(); render();
    const extra = document.querySelector<HTMLDetailsElement>('#extra-events');
    if (extra) extra.open = false;
    if (draft.kind === 'weight' || draft.kind === 'temperature' || draft.kind === 'sunbath') document.querySelector<HTMLElement>('#extra-events summary')?.focus({ preventScroll: true });
  }));
...
function captureForm() {
  ...
  if (input('sunbath-duration')) draft.sunbathDuration = input('sunbath-duration')!.value;
