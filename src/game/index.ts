...
export function mountGame({ container, client, account, onExit, api = createApi(client) }: Options): GameController {
  ...
  const count = () => state?.stats.reduce((sum, s) => sum + s.rounds, 0) ?? 0;
  function stopAnimation() { cancelAnimationFrame(frame); frame = 0; lastFrame = 0; }
  function animate(now: number) {
    if (disposed || hidden || paused || !round || screen !== 'round' || round.awaitingNext || round.quiet || round.game !== 'pacifier') return;
  ...
  function pause() {
    if (round && screen === 'round') { paused = true; stopAnimation(); render(); }
  }
  async function load() {
  ...
  async function begin() {
    ...
    round = startRound(selected, state.unlocks, quiet); paused = false; screen = 'round'; message = ''; render();
  }
  ...
  function homeView() {
    ...
    <div class="game-section-heading"><h2>Ein kleiner Spielmoment</h2><span>5 Aktionen · ohne Eile</span></div>
    <div class="game-cards">${(Object.keys(games) as GameId[]).map(id => {
      ...
      return `<button class="game-card ${!unlocked ? 'game-locked' : ''}" data-game="${id}" ${!unlocked || !canWrite() ? 'disabled' : ''}><span class="game-card-icon">${games[id].icon}</span><span><strong>${games[id].name}</strong><small>${!unlocked ? 'Nach „Schnuller selbst holen“' : id === 'pacifier' && state?.unlocks.includes('self_pacifier') ? 'Zum Spaß üben · immer willkommen' : games[id].subtitle}</small>${stat ? `<small>${stat.rounds} Runden · Bestwert ${stat.best_bonus}/5</small>` : ''}</span><span aria-hidden="true">${unlocked ? '↗' : '⌑'}</span></button>`;
    }).join('')}</div><p class="game-note">${complete ? 'Alle ersten Fähigkeiten entdeckt. Spielt weiter, euer XP bleibt für später.' : 'Jeder kleine Erfolg bringt euch weiter.'}<br>${state?.xp_total ?? 0} XP insgesamt · ${count()} gemeinsame Runden</p>`;
  }
  function skillsView() {
  ...
  function render() {
    ...
    else body = `${baby(!pending)}<h1>${pending ? 'Ein kleiner Erfolg wartet.' : 'Das war ein schöner Moment.'}</h1><p role="status">${pending ? busy ? 'Wird gespeichert …' : 'Noch nicht gespeichert' : escape(message)}</p>${pending ? `<button class="game-primary" data-action="retry" ${busy ? 'disabled' : ''}>${busy ? 'Einen Moment …' : 'Erneut speichern'}</button>` : '<button class="game-primary" data-action="home">Weiter entdecken</button>'}`;
    ...
    container.querySelectorAll<HTMLButtonElement>('[data-action]').forEach(b => b.addEventListener('click', () => {
      switch (b.dataset.action) {
        ...
        case 'home': screen = pending ? 'result' : 'home'; round = null; message = ''; render(); void load(); break;
        ...
        case 'abort': round = null; screen = 'home'; message = ''; render(); break;
    ...
    if (screen === 'round' && !paused) frame = requestAnimationFrame(animate);
  }
