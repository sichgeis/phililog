...
try {
  ...
  const sunbathId = randomUUID(); ids.push(sunbathId);
  const sunbath = { ...input, id: sunbathId, kind: 'sunbath', amount_ml: null, duration_minutes: 5 };
  const beforeSunbath = await julia.client.rpc('daily_report', { first_day: '2026-09-12', last_day: '2026-09-12' });
  assert.ifError(beforeSunbath.error);
  const sunbathCreated = await julia.client.from('feedings').insert(sunbath).select().single();
  assert.ifError(sunbathCreated.error); assert.equal(sunbathCreated.data.duration_minutes, 5);
  const afterSunbath = await julia.client.rpc('daily_report', { first_day: '2026-09-12', last_day: '2026-09-12' });
  assert.ifError(afterSunbath.error);
  assert.deepEqual(afterSunbath.data.map(({ events, ...totals }) => totals), beforeSunbath.data.map(({ events, ...totals }) => totals), 'Sunbath does not change meal/diaper totals');
  assert.equal(afterSunbath.data[0].events, beforeSunbath.data[0].events + 1, 'Sunbath counts as a logged event');
  const sunbathEdited = await christian.client.from('feedings').update({ duration_minutes: 7 }).eq('id', sunbathId).eq('version', 1).select().single();
  assert.ifError(sunbathEdited.error); assert.equal(sunbathEdited.data.duration_minutes, 7);
  assert.deepEqual((await julia.client.from('feedings').update({ duration_minutes: 9 }).eq('id', sunbathId).eq('version', 1).select()).data, []);
  assert.deepEqual((await outsider.client.from('feedings').select().eq('id', sunbathId)).data, []);
  assert.deepEqual((await outsider.client.from('feedings').update({ duration_minutes: 9 }).eq('id', sunbathId).select()).data, []);
  assert.ok((await outsider.client.from('feedings').insert({ ...sunbath, id: randomUUID(), created_by: outsider.user.id })).error);
  assert.ifError((await christian.client.from('feedings').update({ duration_minutes: null }).eq('id', sunbathId)).error);
  for (const extra of [{ duration_minutes: 0 }, { duration_minutes: -1 }, { amount_ml: 65 }, { estimated_ml: 25 }, { mood_after: 'calm' }, { temperature_c: 37.2 }, { weight_g: 3500 }, { urine: true }]) assert.ok((await julia.client.from('feedings').insert({ ...sunbath, id: randomUUID(), ...extra })).error, 'Invalid sunbath details denied');
  assert.deepEqual((await christian.client.from('feedings').delete().eq('id', sunbathId).eq('version', 3).select('id')).data, [{ id: sunbathId }]);
  ...
  try {
    ...
    const recovered = await api.createFeeding(pending, julia.user.id);
    assert.equal(recovered.amount_ml, 70, 'Lost response retry preserves intervening correction');
    assert.equal(recovered.version, corrected.version);
    await api.deleteFeeding(corrected);
    assert.equal(await api.createFeeding(pending, julia.user.id), null, 'Deleted retry is resolved without resurrection');
    assert.equal((await julia.client.from('feedings').select().eq('id', pending.id)).data.length, 0);
    assert.equal((await julia.client.rpc('feeding_create_known', { entry_id: pending.id })).data, true);
    assert.equal((await christian.client.rpc('feeding_create_known', { entry_id: pending.id })).data, false);
    assert.equal((await outsider.client.rpc('feeding_create_known', { entry_id: pending.id })).data, false);
    assert.ok((await anonymous.rpc('feeding_create_known', { entry_id: pending.id })).error);
    assert.ok((await julia.client.schema('private').from('feeding_create_receipts').select()).error);
    const invalidPending = { id: randomUUID(), input: { ...payload, amount_ml: 63 } };
    await assert.rejects(api.createFeeding(invalidPending, julia.user.id));
    assert.equal((await julia.client.rpc('feeding_create_known', { entry_id: invalidPending.id })).data, false, 'Rejected insert leaves no receipt');
