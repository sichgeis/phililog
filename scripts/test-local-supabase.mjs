...
try {
  ...
  for (const activityKind of ['sunbath', 'massage', 'gymnastics']) {
    const activityId = randomUUID(); ids.push(activityId);
    const activity = { ...input, id: activityId, kind: activityKind, amount_ml: null, duration_minutes: 5 };
    const beforeActivity = await julia.client.rpc('daily_report', { first_day: '2026-09-12', last_day: '2026-09-12' });
    assert.ifError(beforeActivity.error);
    const activityCreated = await julia.client.from('feedings').insert(activity).select().single();
    assert.ifError(activityCreated.error); assert.equal(activityCreated.data.duration_minutes, 5);
    const afterActivity = await julia.client.rpc('daily_report', { first_day: '2026-09-12', last_day: '2026-09-12' });
    assert.ifError(afterActivity.error);
    assert.deepEqual(afterActivity.data.map(({ events, ...totals }) => totals), beforeActivity.data.map(({ events, ...totals }) => totals), 'Activity does not change meal/diaper totals');
    assert.equal(afterActivity.data[0].events, beforeActivity.data[0].events + 1, 'Activity counts as a logged event');
    const activityEdited = await christian.client.from('feedings').update({ duration_minutes: 7 }).eq('id', activityId).eq('version', 1).select().single();
    assert.ifError(activityEdited.error); assert.equal(activityEdited.data.duration_minutes, 7);
    assert.deepEqual((await julia.client.from('feedings').update({ duration_minutes: 9 }).eq('id', activityId).eq('version', 1).select()).data, []);
    assert.deepEqual((await outsider.client.from('feedings').select().eq('id', activityId)).data, []);
